...
utils.subBufferString = function (_startIndex, _length, _data) {
    ...
    return data.trim();     //return String.trim(data);
}
...
//Light.prototype
//var
utils.sendUDPWrite = function (light, commandUpper, commandLower, payload, broadcastingToGroup) {
    let groupNumber = 0xff;

    let response = [0xFC, (0x05 + payload.length), groupNumber];
    ...
    if (broadcastingToGroup) {  //broadcast
        light.client.sendMessage(buf, light.client.broadcastIp);
    } else {    //unicast
        light.client.sendMessage(buf, light.ipAddress);
    }
}
